...
# ============================================================================
# 这是 FirstCoder 的组装工厂 (composition root)。
# create_firstcoder_app() 把所有子系统拼成一个可运行的 FirstCoderApp：
...
# → 上一步阅读：firstcoder/cli.py (main, create_cli_app)
# → 下一步阅读：firstcoder/app/runtime.py (AgentChatRunner)
# ============================================================================
...
# adapt_mcp_tool() 来自 firstcoder/mcp/adapter.py，负责把 MCP 协议的工具描述
# 适配成 FirstCoder 内部的 Tool 接口。
# ----------------------------------------------------------------------------
...
# ============================================================================
# create_firstcoder_app() — 整个 TUI 的组装入口 (composition root)
# ============================================================================
# 参数说明：
#   project_root       — 项目根目录，用于解析配置、决定 sandbox 工作目录
#   data_root          — 数据根目录（session/context 持久化），默认 <project_root>/.firstcoder
...
# 返回值：FirstCoderApp — 可直接交给 Textual 运行的 TUI 实例
# ============================================================================
def create_firstcoder_app(
    *,
    ...
    # create_provider_for_model() 来自 firstcoder/providers/factory.py，
    ...
    # ------------------------------------------------------------------
    # create_builtin_registry() 来自 firstcoder/tools/builtin.py，注册所有内置工具
    ...
    # ------------------------------------------------------------------
    # SessionBootstrap 来自 firstcoder/session/bootstrap.py，封装了 session 创建/恢复
    # 的全部逻辑。这里只决定走 resume 还是 from_project 分支。
    # → 见 firstcoder/session/bootstrap.py 了解完整的 session 生命周期。
    ...
    # ------------------------------------------------------------------
    # ContextWindowManager 来自 firstcoder/context/manager.py，负责在 token 预算
    ...
    context_manager = ContextWindowManager( # 上下文压缩管理 → firstcoder/context/manager.py
        store=store,
    ...
    # ------------------------------------------------------------------
    # AgentChatRunner 来自 firstcoder/app/runtime.py，是用户消息到 AgentLoop 执行的桥梁。
    # 它持有 provider、tools、context_manager，负责编排一次完整的 agent 对话循环。
    chat_runner = AgentChatRunner( # 用户发消息 到 AgentLoop 执行的桥梁 → firstcoder/app/runtime.py
    ...
    # 由 FirstCoderApp 在用户输入 "/" 前缀时路由到对应的 handler。
    ...
    command_handler = CompositeCommandHandler( # 路由所有 slash commands → firstcoder/app/router.py
        [
            HelpCommandHandler(),
...
# 配置开关（config 中的 streaming 字段或 FIRSTCODER_STREAMING 环境变量）。
