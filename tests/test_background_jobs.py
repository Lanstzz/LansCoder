...
def test_completed_job_projects_as_notification_not_second_tool_result(tmp_path) -> None:
    ...
    try:
        ...
        tool_messages = [m for m in view.messages if m.role == "tool"]
        assert len(tool_messages) == 1
        notification_messages = [m for m in view.messages if m.role == "notification" and "<task_notification>" in m.parts[0].content]
        ...
        assert sum(1 for m in view_again.messages if m.role == "notification" and "<task_notification>" in m.parts[0].content) == 1
...
def test_task_plan_completion_callback_failure_is_not_reported_as_completed(tmp_path, monkeypatch) -> None:
    ...
    try:
        ...
        assert plan.revision == 1
        assert plan.tasks[0].status == "in_progress"
        notification = [message.parts[0].content for message in session.rebuild_view().messages if message.role == "notification" and "<task_notification>" in message.parts[0].content][0]
...
def test_shared_background_manager_isolates_jobs_by_session(tmp_path) -> None:
    ...
    try:
        ...
        assert not any(message.role == "notification" and "<task_notification>" in message.parts[0].content for message in view_b.messages)
        ...
        assert sum(1 for message in view_a.messages if message.role == "notification" and "<task_notification>" in message.parts[0].content) == 1
    finally:
        release.set()
        manager.shutdown()
