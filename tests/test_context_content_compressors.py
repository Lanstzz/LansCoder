...
def test_old_task_compressor_marks_part_as_micro_compacted() -> None:
    part = MessagePart(
        id="part_1",
        message_id="msg_1",
        kind="text",
    ...
    assert compacted.metadata["compaction_state"] == "micro_compacted"
    assert compacted.metadata["compacted_by"] == "l1_old_task"
    assert "旧任务内容" not in compacted.content
...
def test_cold_text_compressor_marks_part_as_route_compacted() -> None:
    ...
    assert compacted.metadata["compacted_by"] == "l3_current_task_cold"
