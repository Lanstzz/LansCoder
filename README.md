...
当前项目处于骨架阶段。旧的 agent / memory / context 试验实现已经清理，当前工作区先保留稳定的 provider、tool、config 和 utils 基础层，并开始接入最小 TUI 命令入口：
...
- `firstcoder/app`：最小 Textual TUI 外壳和 `/context`、`/compact status`、`/compact` 命令处理层。
- `tests`：测试代码。
