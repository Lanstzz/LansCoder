...
当前项目已经完成第一轮 agent/session/context 闭环：provider、tool、JSONL session store、上下文投影、压缩恢复、只读分享和最小 TUI 命令入口已经接入。代码仍以学习和迭代为主，优先保持简单清晰。
...
- `firstcoder/context`：append-only session event log、上下文投影、checkpoint、archive 和分层压缩。
- `firstcoder/session`：session catalog、resume service、metadata、只读 transcript、share 和脱敏。
- `firstcoder/agent`：单个运行期 `AgentSession` 和一轮 `AgentLoop` 编排。
- `firstcoder/app`：Textual TUI 外壳、普通聊天入口，以及 `/sessions`、`/session`、`/resume`、`/share`、`/rename`、`/context`、`/compact status`、`/compact` 命令处理层。
- `tests`：测试代码。

当前上下文管理、压缩恢复和 session 边界设计说明见：

- `firstcoder/context/ARCHITECTURE.md`：当前 context 上下文管理与压缩架构说明。
- `docs/session-context-plan.md`：session 与 context 的边界、resume 列表和只读分享计划。
...
- 展示工具调用过程和执行结果。
- 后续支持受控的文件读取、编辑和命令执行。
- 支持分层上下文压缩：轻量压缩旧工具输出、按内容类型路由压缩大段输出、归档超大工具结果，并在上下文压力过高时总结旧历史。
...
这些逻辑属于上下文管理，放在 `firstcoder/context` 中，不绑定具体 provider，也不写进 Textual widget。

更详细的当前实现说明见 `firstcoder/context/ARCHITECTURE.md`。
...
- OpenAI-compatible 流式文本输出和 function tool call delta 拼装。
- AgentLoop/TUI runtime 可以消费内部 `ChatStreamEvent`；UI 不直接解析 OpenAI 原始 chunk。
