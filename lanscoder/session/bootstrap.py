...
#
# → 来自：lanscoder/app/factory.py (create_lanscoder_app 中创建 SessionBootstrap)
# → 下游：lanscoder/agent/session.py (AgentSession.create / .resume / .from_project)
# ============================================================================
...
class SessionBootstrap:
    ...
    #   - create_project_permission_manager 来自 lanscoder/agent/session.py，
    #     它按 project_root 建立项目级的权限策略（哪些路径可写/可执行等）
    #   - FilePermissionGrantStore 来自 lanscoder/permissions/grants.py，
    ...
    # ----------------------------------------------------------------------------
    # 新建 session 路径：
    #   - AgentSession.create 来自 lanscoder/agent/session.py
    #   - 读取项目根目录下的 AGENTS.md（read_agents_md 来自
    #     lanscoder/agent/prompt_inputs.py），作为 system prompt 的项目级指令
    #   - 发现项目可用 skills（discover_all_skills 来自
    #     lanscoder/skills/discovery.py），构建 skill_catalog
    #   - 若未指定 session_id，则由 new_session_id() 生成
    #     (lanscoder/context/identity.py)
    # ----------------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------------
    # 恢复 session 路径：
    #   - AgentSession.resume 来自 lanscoder/agent/session.py
    ...
    #   - AgentSession.from_project 来自 lanscoder/agent/session.py
    #   - 注意：这里不传 agents_md 和 skill_catalog —— AgentSession.from_project
    #     内部会基于 project_root 自行读取/发现，避免在 bootstrap 层重复 IO
    #   - 这是 create_lanscoder_app (lanscoder/app/factory.py) 启动 app 时
    #     默认走的 session 组装路径
    # ----------------------------------------------------------------------------
