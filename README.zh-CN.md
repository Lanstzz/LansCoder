...
| 适合谁 | 你能看到什么 |
| --- | --- |
| 想学 agent 内部机制的人 | provider 调用、工具系统、权限、session replay、context projection 的可读实现 |
| 想做简历项目的人 | 一个不只是“调了 LLM API”的完整工程样本 |
| 想研究长会话的人 | 任务边界触发压缩、append-only 事件和 resume 友好的运行状态 |
...
| 可复用工作流指令怎么被发现、路由和加载 | `firstcoder/skills` |
...
| `TASK_HASH_CHANGED` 触发 | 压缩可以因为任务切换发生，而不只是因为 token 快满了 |
| append-only 事件 | resume 可以重放任务边界观察和 active task 状态 |

简单说，FirstCoder 想把压缩从“上下文快爆了才救火”，推进成“跟随任务生命周期主动整理上下文”。
...
| 技能系统 | 项目级和机器级 workflow skill 的发现、路由、加载和审计事件 |
...
FirstCoder 现在有一层一等的 skill 系统，用来承载可复用的工作流指令。Skill 不只是写在 `AGENTS.md` 里的额外提示词：它会被发现、路由、在 provider 工作前加载，并写入 session 日志作为可审计证据。

支持的 skill 来源：

| 来源 | 路径 |
| --- | --- |
| 项目 markdown skill | `<project-root>/skills/*.md` |
| 项目 agent skill | `<project-root>/.agents/skills/*/SKILL.md` |
| 机器级 agent skill | `~/.agents/skills/*/SKILL.md`、`~/.codex/skills/*/SKILL.md` |
| 机器级 markdown skill | `~/.firstcoder/skills/*.md` |

当用户消息高置信匹配某个 skill 时，FirstCoder 会在本轮第一次 provider request 前加载完整 skill 文件。Skill 也可以声明必读文件，这些文件会从同一个 skill root 内读取，并和已加载 skill 一起进入上下文。

Skill 行为会通过 append-only session event 留痕：
...
项目 skill 优先于全局 skill。全局 skill 可以补充这台机器上的长期能力，但不能覆盖项目规则、权限策略或 sandbox 边界。
