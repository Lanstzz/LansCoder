...
| 代码形态 | 当前 `firstcoder/` 约 2.3 万行 Python（约 170 个文件） | TS/JS 代码规模约 57 万行，平台层和工程表面也更多 |
...
Provider 主线同时覆盖 OpenAI Chat Completions 兼容路径与原生 Anthropic Messages API；二者在 complete/streaming、tools、forced `tool_choice`、usage 与错误归类上对齐同一内部契约。
