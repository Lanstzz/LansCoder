...
| O10 | **LoCoBench 场景 ground_truth 与提供的项目文件不一致(数据质量问题)**——expert 场景最严重。**根因(生成管线两阶段解耦)**:Phase 2 `SyntheticProjectGenerator` 先独立生成代码(`data/generated/<project_id>/`);Phase 3 `ScenarioGenerator` 再生成场景+ground_truth,但 prompt 只给 `AVAILABLE FILES`(路径+行数+字符数),**不给代码内容**,并让 LLM 编"realistic and challenging"的细节 → ground_truth 里的标识符(`PIPELINE_ROUTING`/`EventStandardizerV3`/`type_gamma`/"Gamma Processing Pipeline")是**想象出来的**,从未与代码交叉校验;原始场景 `context_files` 还混入 `name`/`children`/`type` 键(序列化脏),`gamma` 在整仓 0 次出现 | `python_data_streaming_expert_085_code_comprehension_expert_01`:ground_truth 引用的全部关键标识符在提供的 83 个文件里都不存在(config.py 无 `PIPELINE_ROUTING`/`type_gamma`、module_62 无 `EventStandardizerV3`/`_normalize_timestamps`、module_27 无 `validate_schema_v1`、utils.py 无 `DateUtil`);全数据集 grep 只有 scenario JSON 含这些标识符,无任何源文件;agent 实际读到的 config.py(14316 字符)与缓存一致。easy 7/13、hard 11/21 也有 ground_truth 标识符缺失 | **expert 的 B correctness=0 度量的是数据 bug 而非 agent**:agent 依据真实文件找到 module_21 `_MINIMAL_SCHEMA`(date-time)并修复,结论对"给出的代码"合理。Phase 3.5 B 层 judge 必须知情"代码可能与 ground_truth 不一致"或先做一致性校验;ground_truth↔文件不符的场景应标注 B correctness 不可比 |
| O11 | L2 归档导致上下文抖动(retrieve 风暴) | expert 6 次 L2 压缩归档 331 个工具结果,agent 601 次 `retrieve_archive`(turn1 220/turn2 223),turn1/2 撞 60 轮工具上限被截断;归档/取回反复 ping-pong | 与 P1/P2 相关:压缩策略需考虑检索往返成本;Phase 4 A/B 应把 retrieve 次数/工具轮耗尽作为效率指标之一 |
| O12 | expert 场景基模表现本身合理(轻微推理缺口) | agent 读完全部 73 模块 + config/utils,正确找到真实存在的 `_MINIMAL_SCHEMA`(date-time)并修复,还发现 module_46 次要点;但把 module_62"Transform Strategy Registry"一带而过,未深挖转换管线是否会改写时间戳 | 非主因;若代码与 ground_truth 一致,是否追 module_62 才真正检验基模——需用一致场景再评 |
