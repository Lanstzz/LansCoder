...
| O10 | **LoCoBench 场景 ground_truth 与提供的项目文件不一致(数据质量问题)**——expert 场景最严重 | `python_data_streaming_expert_085_code_comprehension_expert_01`:ground_truth 引用的全部关键标识符在提供的 83 个文件里都不存在(config.py 无 `PIPELINE_ROUTING`/`type_gamma`、module_62 无 `EventStandardizerV3`/`_normalize_timestamps`、module_27 无 `validate_schema_v1`、utils.py 无 `DateUtil`);全数据集 grep 只有 scenario JSON 含这些标识符,无任何源文件;agent 实际读到的 config.py(14316 字符)与缓存一致。easy 7/13、hard 11/21 也有 ground_truth 标识符缺失 | **expert 的 B correctness=0 度量的是数据 bug 而非 agent**:agent 依据真实文件找到 module_21 `_MINIMAL_SCHEMA`(date-time)并修复,结论对"给出的代码"合理。Phase 3.5 B 层 judge 必须知情"代码可能与 ground_truth 不一致"或先做一致性校验;ground_truth↔文件不符的场景应标注 B correctness 不可比 |
