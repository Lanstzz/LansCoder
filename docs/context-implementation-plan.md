...
```text
1. 新增 tests/test_context_checkpoint.py 和 tests/test_context_resume.py。
2. 实现 firstcoder/context/checkpoint.py。
3. 让 ContextBuilder 支持 latest checkpoint summary + recent tail 投影。
4. 确认 checkpoint 覆盖过的旧历史不会重复投影。
5. 跑 python -m pytest。
```
